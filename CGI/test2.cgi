#!/bin/bash
process=`echo "$QUERY_STRING" | awk -F= '{print $2}' | cut -d "=" -f1`
server=`echo "$QUERY_STRING"| awk -F= '{print $1}' | cut -d "=" -f1`
...
then
ssh microbus@$server ". ~/bin/ENV_PROD1_LOCAL;cd /home/microbus/Tool;./sentry.sh $process $server"
...
then
ssh microbus@$server ". ~/bin/ENV_TEST1_LOCAL;cd /home/microbus/Tool;./sentry.sh $process $server"
